...
import { Search, MessageSquare, Bell, Loader2 } from "lucide-react";
...
const Messages = () => {
  ...
  const { notifications, unreadCount: notifUnread, markAsRead } = useNotifications("professional");
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      ...
      try {
        ...
        const dataWithCustomers = (data || []).map(c => ({ ...c, customer: customerMap[c.customer_id] || undefined }));

        const conversationsWithMessages = await Promise.all(
          dataWithCustomers.map(async (conv) => {
            const { data: messages } = await supabase
              .from("messages").select("content, read_at")
            ...
            return { ...conv, lastMessage: messages?.[0]?.content || "No messages yet", unreadCount: count || 0 };
          })
        );
  ...
  const totalChatUnread = conversations.reduce((sum, c) => sum + (c.unreadCount || 0), 0);
  const totalBadge = totalChatUnread + notifUnread;
  ...
  // Vibrate on new unread
  useEffect(() => {
    if (totalBadge > 0 && navigator.vibrate) {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <AppHeader title="Messages" notificationCount={totalBadge} />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="notifications" className="relative">
              Notifications
              {notifUnread > 0 && (
                <span className="ml-2 w-5 h-5 bg-destructive text-white text-[10px] font-bold rounded-full flex items-center justify-center">
          ...
          <TabsContent value="chats" className="mt-4 space-y-1">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground mt-1">Messages from customers will appear here</p>
              </div>
            ) : (
              filteredConversations.map((conversation) => (
                ...
                >
                  <Avatar className="w-12 h-12">
                    <AvatarImage src={conversation.customer?.avatar_url || undefined} className="object-cover" />
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="font-medium text-sm truncate">{conversation.customer?.full_name || "Customer"}</h3>
