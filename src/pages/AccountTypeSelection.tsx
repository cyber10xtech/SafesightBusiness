  ...
  const accountTypes = [
    {
      ...
      features: ["Licensed & certified", "Business registration (CAC)", "Verified badge on profile", "Higher booking rates"],
    ...
    },
    {
      ...
      features: ["Skilled & experienced", "Shop/workspace photo", "Quick onboarding", "Flexible job options"],
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <div className="gradient-primary px-6 py-10 text-center">
        <div className="w-16 h-16 rounded-xl overflow-hidden mx-auto mb-4 shadow-lg icon-float">
          <img src={logoBusiness} alt="Safesight" className="w-full h-full object-cover" />
        </div>
        <h1 className="text-3xl font-bold text-white mb-2 animate-fade-in">Choose Your Account Type</h1>
        <p className="text-white/80 animate-fade-in" style={{ animationDelay: "0.1s" }}>Select the option that best describes you</p>
      </div>
        ...
        <div className="max-w-md mx-auto space-y-4">
          {accountTypes.map((type, index) => (
            <button
              ...
              className={cn(
                "w-full text-left p-5 rounded-2xl border-2 transition-all animate-fade-in",
                selectedType === type.id ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"
              )}
            ...
            >
              ...
              <div className="pt-3 border-t border-border">
                <p className="text-xs text-muted-foreground"><span className="font-medium">Examples:</span> {type.examples}</p>
              </div>
            </button>
          ))}
          ...
          <div className="flex gap-3 pt-4">
            <Button variant="outline" onClick={() => navigate("/")} className="flex-1 h-12 rounded-xl">Back</Button>
            <Button
              onClick={() => navigate("/register", { state: { accountType: selectedType } })}
              disabled={!selectedType}
              className="flex-1 h-12 bg-primary text-primary-foreground rounded-xl tap-scale"
            >
              Continue
